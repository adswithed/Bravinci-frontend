...
import { ArrowRight, Shield, Award, Users, Globe, Sparkles } from 'lucide-react'
import gsap from 'gsap'
import Link from 'next/link'

const stats = [
  { icon: Shield, label: 'Strategic Transformation', value: '15+ Years' },
  { icon: Award, label: 'Solutions Deployed', value: '200+' },
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="relative z-10 space-y-4 lg:space-y-6 text-center lg:text-left">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 backdrop-blur-sm border border-white/10 text-sm font-medium">
              <Globe className="w-4 h-4 text-[#0E78AA]" />
              <span className="text-white/90">Global Strategic Intelligence</span>
              <Sparkles className="w-3 h-3 text-[#F7AE57]" />
            </div>
            ...
            <div
              ref={statsRef}
              className="flex flex-wrap gap-4 lg:gap-5 pt-4 lg:pt-6 border-t border-white/10 justify-center lg:justify-start"
            >
              {stats.map((stat, index) => (
                ...
                >
                  <div className="p-1.5 rounded-lg bg-[#0E78AA]/20 border border-[#0E78AA]/30">
                    <stat.icon className="w-4 h-4 text-[#0E78AA]" />
                  </div>
          ...
          >
            ...
            <div className="relative w-full max-w-[450px] lg:max-w-[500px] aspect-square">
              ...
              {/* Floating labels */}
              <div className="absolute top-8 right-8 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-xs text-white/80 animate-pulse">
                <span className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#F7AE57]" />
                  6 Continents
                </span>
              </div>

              <div className="absolute bottom-16 left-4 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-xs text-white/80">
                <span className="flex items-center gap-1.5">
              ...
              {/* Interactive hint */}
              <div className="absolute bottom-2 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-full bg-black/40 backdrop-blur-sm border border-white/10 text-xs text-white/50">
                Drag to rotate
              </div>
            </div>
          </div>
